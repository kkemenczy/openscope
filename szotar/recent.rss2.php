...
while ( $terms_count < 15 && $i > 0 ) {
    ...
    list ( $term['name'], $term['translation'], $term['context'], $term['date'], $term['author']) = explode("\t", $lines[$i--], 5);
    if (!in_array( $term['name'], $added_terms )) {
        ...
        list ( $date['year'], $date['month'], $date['day'], $date['hour'], $date['minute'], $date['second'] ) = explode ('[ \:\-]',$term['date'],6);
